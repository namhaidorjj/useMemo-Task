...
export default function Home() {
  ...
  const totalPrice = () => {
    return Array.from(data).reduceRight((acc, cur) => acc + cur.followers, 0);
  };

  function addCart() {}

  const totalFollowers = useMemo(() => totalPrice(), [data]);
      ...
      <div>
        {articles.map((article) => (
          <div className="flex gap-20">
            <p className="w-[700px] h-[20px]">{article.title}</p>
            <p>{article.userId}</p>
            <button
              onClick={addCart}
              className="rounded-xl w-[100px] h-[20px] border-2 flex items-center justify-center">
              ADD Cart
            </button>
            <div className="bg-red-200 w-[]"></div>
          </div>
        ))}
      </div>
      <div>
